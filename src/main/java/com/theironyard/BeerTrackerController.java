...
public class BeerTrackerController {
    ...
    public void init() throws InvalidKeySpecException, NoSuchAlgorithmException {
        User user = users.findOneByName("Alex");
        if (user == null) {
            user = new User();
            user.name = "Alex";
            user.password = PasswordHash.createHash("hanger");
            users.save(user);
        }
    }
    ...
    public String editBeer (int id, String name, String type, Integer beercalories, HttpSession session) throws Exception {
